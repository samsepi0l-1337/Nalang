...
def hint_for_devid(value: int) -> str:
    ...
    return (
        ...
        "센서 I2C 핀을 Pi I2C에 붙이지 않았는지 확인. docs/wiring.md."
    )


HINT_SPI_MISSING = (
    "/dev/spidev0.0 없음. raspi-config에서 SPI Enable 후 재부팅. "
    "ls -l /dev/spidev0.0. docs/wiring.md OS."
)
HINT_SPI_PERM = "sudo usermod -aG spi,i2c $USER 후 재로그인. docs/wiring.md OS."
HINT_SPIDEV_PKG = "Pi에서 pip install -r requirements-pi.txt (spidev). docs/logs.md."
HINT_LCD = (
    "i2cdetect -y 1 에 0x27 또는 0x3F가 있는지 확인. "
    "SDA(핀3) SCL(핀5) GND(핀6) 백팩 VCC=3.3V(핀17, 5V 금지). docs/wiring.md LCD."
)
HINT_SAMPLE = "센서 SPI 배선·납땜과 DEVID 로그를 확인. docs/wiring.md 센서 SPI."
HINT_LCD_WRITE = "I2C 커넥터·백팩 3.3V·주소 0x27/0x3F. docs/wiring.md LCD."
